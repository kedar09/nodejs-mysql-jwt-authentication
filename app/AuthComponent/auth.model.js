...
exports.loginUser = function (req, result) {
    connection.query("select count(*) as count,any_value(userId) as userId,email,password from users where email=? group by password limit 1",
            ...
            if (err) {
                ...
                if (records.length > 0) {
                    if (bcrypt.compareSync(req.body.password, records[0].password)) {
                        ...
                        let token = jwt.sign({payload}, JWTPrivateKey.privateKey, {expiresIn: '1d'});
                        let resultLoginUser = {
...
exports.registerUser = function (req, result) {
    connection.query("select count(*) as count from users where email=?",
...
exports.resetPasswordLink = function (req, result) {
    connection.query("select count(*) as count,userId from users where email=?",
        [req.body.email], function (error, records) {
            if (error) {
                result(error, null);
            } else {
...
exports.updateUserPassword = function (req, result){
    ...
    let token = req.headers.referer.substring(tokenStartLength+1, tokenLastLength);
